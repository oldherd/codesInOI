...
struct Splay{
	...
	int newnode(int v, int f) {
		ch[tot][0] = ch[tot][1] = 0; size[tot] = cnt[tot] = 1;
	...
	void find(int v) {
		int cur = root;
		while(ch[cur][v>val[cur]] && val[cur] != v) cur = ch[cur][v>val[cur]];
		splay(cur);
	}
	void insert(int v) {
		int cur = root;
		while(ch[cur][v>val[cur]] && val[cur] != v) cur = ch[cur][v>val[cur]];
		if(val[cur] == v) cnt[cur]++;
		else cur = ch[cur][v>val[cur]] = newnode(v, cur);
		splay(cur);
	}
	int prePost(int v, int f) {
		find(v);
		if((val[root]<v && !f) || (v<val[root] && f)) return root;
	...
	int parti(int cur, int k) {
		if(size[ch[cur][0]] < k && size[ch[cur][0]]+cnt[cur] >= k) return val[cur];
